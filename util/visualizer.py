...
def renorm(
    ...
    if img.dim() == 3:
        ...
        mean = torch.Tensor(mean).to(img_perm.device)
        std = torch.Tensor(std).to(img_perm.device)
...
class COCOVisualizer:
    ...
    def visualize(
        self, img, tgt, caption=None, dpi=120, savedir=None, show_in_console=True
    ):
        """
        img: tensor(3, H, W)
    ...
    def addtgt(self, tgt):
        ...
        for box in tgt["boxes"].cpu():
            ...
            bbox_x, bbox_y, bbox_w, bbox_h=x1,y1,x1+(x2-x1)/2,y1+(y2-y1)/2
