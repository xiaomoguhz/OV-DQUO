...
def multi_head_attention_forward(query: Tensor,
    ...
    if v is not None:
        v = v.contiguous().view(-1, bsz * num_heads, v_head_dim).transpose(0, 1) # L(NH)D
...
def multi_head_attention_forward_trans(query: Tensor,
                                 key: Tensor,
                                 value: Tensor,
                                 embed_dim_to_check: int,
                                 num_heads: int,
    ...
    assert embed_dim == embed_dim_to_check
    # allow MHA to have different sizes for the feature dimension
    assert key.size(0) == value.size(0) and key.size(2) == value.size(2)
    ...
    q = q.contiguous().view(bsz * num_heads, head_dim, tgt_len)
    if k is not None:
        k = k.contiguous().view(bsz * num_heads, head_dim, -1)
    if v is not None:
        v = v.contiguous().view(bsz * num_heads, v_head_dim, -1)

    src_len = k.size(0)

    attn_output_weights = torch.bmm(q.transpose(1,2), k)
    # assert list(attn_output_weights.size()) == [bsz * num_heads, tgt_len, src_len]
