...
class CollateFn:
    ...
    def __call__(self, batch):
        ...
        batch[0] = self.nested_tensor_from_tensor_list_fixed(batch[0])
        return tuple(batch)

    def nested_tensor_from_tensor_list_fixed(self, tensor_list: List[Tensor]):
        # TODO make this more general
        if tensor_list[0].ndim == 3:
            if torchvision._is_tracing():
                return _onnx_nested_tensor_from_tensor_list(tensor_list)
            max_size = [3,self.resolution[0],self.resolution[1]]
